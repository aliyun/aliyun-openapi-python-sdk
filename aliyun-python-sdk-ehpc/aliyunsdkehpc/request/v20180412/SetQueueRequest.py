...
class SetQueueRequest(RpcRequest):
	...
	def get_Nodes(self): # RepeatList
		return self.get_query_params().get('Node')

	def set_Nodes(self, Node):  # RepeatList
		for depth1 in range(len(Node)):
			if Node[depth1].get('Name') is not None:
				self.add_query_param('Node.' + str(depth1 + 1) + '.Name', Node[depth1].get('Name'))
