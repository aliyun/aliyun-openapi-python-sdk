...
class ModifyIpStatusRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'CCIoTGW', '2021-07-21', 'ModifyIpStatus')
		self.set_method('GET')
	...
	def get_IpLists(self): # RepeatList
		return self.get_query_params().get('IpList')

	def set_IpLists(self, IpList):  # RepeatList
		for depth1 in range(len(IpList)):
			self.add_query_param('IpList.' + str(depth1 + 1), IpList[depth1])
