...
class UpdateServerGroupServersAttributeRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Nlb', '2022-04-30', 'UpdateServerGroupServersAttribute','nlb')
	...
	def set_Serverss(self, Servers):  # RepeatList
		for depth1 in range(len(Servers)):
			...
			if Servers[depth1].get('Weight') is not None:
				self.add_body_params('Servers.' + str(depth1 + 1) + '.Weight', Servers[depth1].get('Weight'))
			if Servers[depth1].get('Description') is not None:
				self.add_body_params('Servers.' + str(depth1 + 1) + '.Description', Servers[depth1].get('Description'))
