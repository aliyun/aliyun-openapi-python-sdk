...
class UpdateLoadBalancerZonesRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Nlb', '2022-04-30', 'UpdateLoadBalancerZones','nlb')
	...
	def set_ZoneMappingss(self, ZoneMappings):  # RepeatList
		for depth1 in range(len(ZoneMappings)):
			...
			if ZoneMappings[depth1].get('PrivateIPv4Address') is not None:
				self.add_body_params('ZoneMappings.' + str(depth1 + 1) + '.PrivateIPv4Address', ZoneMappings[depth1].get('PrivateIPv4Address'))
