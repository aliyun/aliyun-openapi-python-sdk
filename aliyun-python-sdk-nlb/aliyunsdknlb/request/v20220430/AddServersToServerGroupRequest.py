...
class AddServersToServerGroupRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Nlb', '2022-04-30', 'AddServersToServerGroup','nlb')
	...
	def set_Servers(self, Servers):  # Array
		for index1, value1 in enumerate(Servers):
			...
			if value1.get('Weight') is not None:
				self.add_body_params('Servers.' + str(index1 + 1) + '.Weight', value1.get('Weight'))
			if value1.get('Description') is not None:
				self.add_body_params('Servers.' + str(index1 + 1) + '.Description', value1.get('Description'))
