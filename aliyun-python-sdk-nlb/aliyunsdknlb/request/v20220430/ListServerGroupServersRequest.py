...
class ListServerGroupServersRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Nlb', '2022-04-30', 'ListServerGroupServers','nlb')
	...
	def get_ServerIdss(self): # RepeatList
		return self.get_body_params().get('ServerIds')

	def set_ServerIdss(self, ServerIds):  # RepeatList
		for depth1 in range(len(ServerIds)):
			self.add_body_params('ServerIds.' + str(depth1 + 1), ServerIds[depth1])
	def get_ServerIpss(self): # RepeatList
		return self.get_body_params().get('ServerIps')

	def set_ServerIpss(self, ServerIps):  # RepeatList
		for depth1 in range(len(ServerIps)):
			self.add_body_params('ServerIps.' + str(depth1 + 1), ServerIps[depth1])
