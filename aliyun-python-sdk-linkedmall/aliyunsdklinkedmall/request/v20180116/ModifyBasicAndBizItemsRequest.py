...
class ModifyBasicAndBizItemsRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'linkedmall', '2018-01-16', 'ModifyBasicAndBizItems','linkedmall')
		...
		for i in range(len(ItemLists)):	
			...
			for j in range(len(ItemLists[i].get('SkuLists'))):
				if ItemLists[i].get('SkuLists')[j] is not None:
					...
					if ItemLists[i].get('SkuLists')[j].get('SupplierPrice') is not None:
						self.add_body_params('ItemList.' + str(i + 1) + '.SkuList.'+str(j + 1)+ '.SupplierPrice', ItemLists[i].get('SkuLists')[j].get('SupplierPrice'))
