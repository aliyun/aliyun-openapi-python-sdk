...
class DeleteGroupTagMiddleRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Oam', '2017-01-01', 'DeleteGroupTagMiddle')
	...
	def set_TagIds(self, TagIds):
		for depth1 in range(len(TagIds)):
			if TagIds[depth1] is not None:
				self.add_query_param('TagId.' + str(depth1 + 1) , TagIds[depth1])
