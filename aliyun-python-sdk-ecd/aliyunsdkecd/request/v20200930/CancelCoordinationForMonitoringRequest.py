...
class CancelCoordinationForMonitoringRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'ecd', '2020-09-30', 'CancelCoordinationForMonitoring')
	...
	def get_CoIdss(self): # RepeatList
		return self.get_query_params().get('CoIds')

	def set_CoIdss(self, CoIds):  # RepeatList
		for depth1 in range(len(CoIds)):
			self.add_query_param('CoIds.' + str(depth1 + 1), CoIds[depth1])
