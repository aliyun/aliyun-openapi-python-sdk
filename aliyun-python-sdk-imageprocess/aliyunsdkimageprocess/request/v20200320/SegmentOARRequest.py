...
class SegmentOARRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'imageprocess', '2020-03-20', 'SegmentOAR','imageprocess')
	...
	def get_BodyPart(self): # String
		return self.get_body_params().get('BodyPart')

	def set_BodyPart(self, BodyPart):  # String
		self.add_body_params('BodyPart', BodyPart)
	...
	def get_Contrast(self): # Boolean
		return self.get_body_params().get('Contrast')

	def set_Contrast(self, Contrast):  # Boolean
		self.add_body_params('Contrast', Contrast)
	def get_MaskLists(self): # RepeatList
		return self.get_body_params().get('MaskList')

	def set_MaskLists(self, MaskList):  # RepeatList
		for depth1 in range(len(MaskList)):
			self.add_body_params('MaskList.' + str(depth1 + 1), MaskList[depth1])
