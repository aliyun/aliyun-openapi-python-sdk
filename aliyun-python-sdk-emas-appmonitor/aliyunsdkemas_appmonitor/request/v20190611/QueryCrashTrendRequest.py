...
class QueryCrashTrendRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'emas-appmonitor', '2019-06-11', 'QueryCrashTrend')
	...
	def get_CrashStatType(self):
		return self.get_body_params().get('CrashStatType')

	def set_CrashStatType(self,CrashStatType):
		self.add_body_params('CrashStatType', CrashStatType)
	...
	def set_ErrorType(self, ErrorTypes):
		for depth1 in range(len(ErrorTypes)):
			if ErrorTypes[depth1] is not None:
				self.add_body_params('ErrorType.' + str(depth1 + 1) , ErrorTypes[depth1])

	def get_ErrorCategory(self):
		return self.get_body_params().get('ErrorCategory')

	def set_ErrorCategory(self,ErrorCategory):
		self.add_body_params('ErrorCategory', ErrorCategory)