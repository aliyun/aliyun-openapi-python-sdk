...
class WithdrawVpcPublishedRouteEntriesRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Vpc', '2016-04-28', 'WithdrawVpcPublishedRouteEntries','vpc')
	...
	def set_RouteEntriess(self, RouteEntries):  # RepeatList
		for depth1 in range(len(RouteEntries)):
			...
			if RouteEntries[depth1].get('DestinationCidrBlock') is not None:
				self.add_query_param('RouteEntries.' + str(depth1 + 1) + '.DestinationCidrBlock', RouteEntries[depth1].get('DestinationCidrBlock'))
