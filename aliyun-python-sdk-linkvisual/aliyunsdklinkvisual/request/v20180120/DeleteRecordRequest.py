...
class DeleteRecordRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Linkvisual', '2018-01-20', 'DeleteRecord','Linkvisual')
	...
	def get_FileNameLists(self): # RepeatList
		return self.get_query_params().get('FileNameList')

	def set_FileNameLists(self, FileNameList):  # RepeatList
		for depth1 in range(len(FileNameList)):
			self.add_query_param('FileNameList.' + str(depth1 + 1), FileNameList[depth1])
