...
class CreateImageRequest(RpcRequest):
	...
	def set_DiskDeviceMapping(self,DiskDeviceMapping):
		for i in range(len(DiskDeviceMapping)):	
			self.add_query_param('DiskDeviceMapping.' + bytes(i + 1) + '.Size' , DiskDeviceMapping[i].get('Size'))
			self.add_query_param('DiskDeviceMapping.' + bytes(i + 1) + '.SnapshotId' , DiskDeviceMapping[i].get('SnapshotId'))
			self.add_query_param('DiskDeviceMapping.' + bytes(i + 1) + '.Device' , DiskDeviceMapping[i].get('Device'))
			self.add_query_param('DiskDeviceMapping.' + bytes(i + 1) + '.DiskType' , DiskDeviceMapping[i].get('DiskType'))
