...
class AssociateAdditionalCertificatesWithListenerRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Ga', '2019-11-20', 'AssociateAdditionalCertificatesWithListener','gaplus')
	...
	def set_Certificatess(self, Certificates):  # RepeatList
		for depth1 in range(len(Certificates)):
			...
			if Certificates[depth1].get('Domain') is not None:
				self.add_query_param('Certificates.' + str(depth1 + 1) + '.Domain', Certificates[depth1].get('Domain'))
