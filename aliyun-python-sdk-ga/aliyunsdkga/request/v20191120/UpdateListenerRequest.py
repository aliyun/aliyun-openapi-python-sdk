...
class UpdateListenerRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Ga', '2019-11-20', 'UpdateListener','ga')
	...
	def get_BackendPortss(self):
		return self.get_query_params().get('BackendPorts')

	def set_BackendPortss(self, BackendPortss):
		for depth1 in range(len(BackendPortss)):
			if BackendPortss[depth1].get('FromPort') is not None:
				self.add_query_param('BackendPorts.' + str(depth1 + 1) + '.FromPort', BackendPortss[depth1].get('FromPort'))
			if BackendPortss[depth1].get('ToPort') is not None:
				self.add_query_param('BackendPorts.' + str(depth1 + 1) + '.ToPort', BackendPortss[depth1].get('ToPort'))
