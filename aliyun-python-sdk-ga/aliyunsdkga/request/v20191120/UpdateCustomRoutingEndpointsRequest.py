...
class UpdateCustomRoutingEndpointsRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Ga', '2019-11-20', 'UpdateCustomRoutingEndpoints','gaplus')
	...
	def set_EndpointConfigurationss(self, EndpointConfigurations):  # RepeatList
		for depth1 in range(len(EndpointConfigurations)):
			...
			if EndpointConfigurations[depth1].get('EndpointId') is not None:
				self.add_query_param('EndpointConfigurations.' + str(depth1 + 1) + '.EndpointId', EndpointConfigurations[depth1].get('EndpointId'))
