...
class UpdateCustomRoutingEndpointTrafficPoliciesRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'Ga', '2019-11-20', 'UpdateCustomRoutingEndpointTrafficPolicies','gaplus')
	...
	def set_PolicyConfigurationss(self, PolicyConfigurations):  # RepeatList
		for depth1 in range(len(PolicyConfigurations)):
			if PolicyConfigurations[depth1].get('PolicyId') is not None:
				self.add_query_param('PolicyConfigurations.' + str(depth1 + 1) + '.PolicyId', PolicyConfigurations[depth1].get('PolicyId'))
