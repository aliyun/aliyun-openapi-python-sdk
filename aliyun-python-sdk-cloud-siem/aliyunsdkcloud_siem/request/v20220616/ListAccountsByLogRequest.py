...
class ListAccountsByLogRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'cloud-siem', '2022-06-16', 'ListAccountsByLog','cloud-siem')
	...
	def get_LogCodess(self): # RepeatList
		return self.get_body_params().get('LogCodes')

	def set_LogCodess(self, LogCodes):  # RepeatList
		for depth1 in range(len(LogCodes)):
			self.add_body_params('LogCodes.' + str(depth1 + 1), LogCodes[depth1])
