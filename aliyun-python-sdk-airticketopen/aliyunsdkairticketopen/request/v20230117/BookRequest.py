...
class BookRequest(RoaRequest):

	def __init__(self):
		RoaRequest.__init__(self, 'airticketOpen', '2023-01-17', 'Book')
		self.set_uri_pattern('/airticket/v1/trade/action-book')
	...
	def get_solution_id(self): # String
		return self.get_body_params().get('solution_id')

	def set_solution_id(self, solution_id):  # String
		self.add_body_params('solution_id', solution_id)
	def get_passenger_ancillary_purchase_map_list(self): # Array
		return self.get_body_params().get('passenger_ancillary_purchase_map_list')

	def set_passenger_ancillary_purchase_map_list(self, passenger_ancillary_purchase_map_list):  # Array
		self.add_body_params("passenger_ancillary_purchase_map_list", json.dumps(passenger_ancillary_purchase_map_list))
	def get_out_order_num(self): # String
		return self.get_body_params().get('out_order_num')

	def set_out_order_num(self, out_order_num):  # String
		self.add_body_params('out_order_num', out_order_num)
	...
	def get_contact(self): # Struct
		return self.get_body_params().get('contact')

	def set_contact(self, contact):  # Struct
		self.add_body_params("contact", json.dumps(contact))
	...
	def get_passenger_list(self): # Array
		return self.get_body_params().get('passenger_list')

	def set_passenger_list(self, passenger_list):  # Array
		self.add_body_params("passenger_list", json.dumps(passenger_list))
