...
class CreateGroupMonitoringAgentProcessRequest(RpcRequest):
	...
	def set_MatchExpresss(self, MatchExpresss):
		for depth1 in range(len(MatchExpresss)):
			if MatchExpresss[depth1].get('Function') is not None:
				self.add_query_param('MatchExpress.' + str(depth1 + 1) + '.Function', MatchExpresss[depth1].get('Function'))
			if MatchExpresss[depth1].get('Name') is not None:
				self.add_query_param('MatchExpress.' + str(depth1 + 1) + '.Name', MatchExpresss[depth1].get('Name'))
			if MatchExpresss[depth1].get('Value') is not None:
				self.add_query_param('MatchExpress.' + str(depth1 + 1) + '.Value', MatchExpresss[depth1].get('Value'))