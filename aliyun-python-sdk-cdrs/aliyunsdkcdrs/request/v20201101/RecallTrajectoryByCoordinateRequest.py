...
class RecallTrajectoryByCoordinateRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'CDRS', '2020-11-01', 'RecallTrajectoryByCoordinate')
	...
	def get_PointList(self):
		return self.get_body_params().get('PointList')

	def set_PointList(self, PointLists):
		for depth1 in range(len(PointLists)):
			if PointLists[depth1].get('Latitude') is not None:
				self.add_body_params('PointList.' + str(depth1 + 1) + '.Latitude', PointLists[depth1].get('Latitude'))
			if PointLists[depth1].get('Longitude') is not None:
				self.add_body_params('PointList.' + str(depth1 + 1) + '.Longitude', PointLists[depth1].get('Longitude'))
