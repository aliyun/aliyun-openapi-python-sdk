...
class ListManagedRacksRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'SmartHosting', '2020-08-01', 'ListManagedRacks','SmartHosting')
	...
	def get_ManagedRackIds(self):
		return self.get_query_params().get('ManagedRackIds')

	def set_ManagedRackIds(self, ManagedRackIds):
		for depth1 in range(len(ManagedRackIds)):
			if ManagedRackIds[depth1] is not None:
				self.add_query_param('ManagedRackId.' + str(depth1 + 1) , ManagedRackIds[depth1])
