...
class CreatePredictiveJobGroupRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'CCC', '2017-07-05', 'CreatePredictiveJobGroup')
	...
	def get_JobsJsons(self):
		return self.get_body_params().get('JobsJsons')

	def set_JobsJsons(self,JobsJsons):
		for i in range(len(JobsJsons)):	
			if JobsJsons[i] is not None:
				self.add_body_params('JobsJson.' + str(i + 1) , JobsJsons[i]);
