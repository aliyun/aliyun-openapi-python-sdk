...
class AssignUsersRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'CCC', '2017-07-05', 'AssignUsers')
	...
	def get_UserRamIds(self):
		return self.get_query_params().get('UserRamId')

	def set_UserRamIds(self, UserRamIds):
		for depth1 in range(len(UserRamIds)):
			if UserRamIds[depth1] is not None:
				self.add_query_param('UserRamId.' + str(depth1 + 1) , UserRamIds[depth1])
