...
class CreateRulesRequest(RpcRequest):
	...
	def __init__(self):
		RpcRequest.__init__(self, 'Alb', '2020-06-16', 'CreateRules','alb')
	...
	def set_Rules(self, Rules):  # Array
		for index1, value1 in enumerate(Rules):
			...
			if value1.get('Tag') is not None:
				for index2, value2 in enumerate(value1.get('Tag')):
					if value2.get('Value') is not None:
						self.add_body_params('Rules.' + str(index1 + 1) + '.Tag.' + str(index2 + 1) + '.Value', value2.get('Value'))
					if value2.get('Key') is not None:
						self.add_body_params('Rules.' + str(index1 + 1) + '.Tag.' + str(index2 + 1) + '.Key', value2.get('Key'))
			...
			if value1.get('Direction') is not None:
				self.add_body_params('Rules.' + str(index1 + 1) + '.Direction', value1.get('Direction'))
