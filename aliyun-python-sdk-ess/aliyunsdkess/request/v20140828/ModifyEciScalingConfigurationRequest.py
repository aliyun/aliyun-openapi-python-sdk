		...
		for depth1 in range(len(Containers)):
			...
			if Containers[depth1].get('EnvironmentVar') is not None:
				for depth2 in range(len(Containers[depth1].get('EnvironmentVar'))):
					if Containers[depth1].get('EnvironmentVar')[depth2].get('FieldRef.FieldPath') is not None:
						self.add_query_param('Container.' + str(depth1 + 1) + '.EnvironmentVar.' + str(depth2 + 1) + '.FieldRef.FieldPath', Containers[depth1].get('EnvironmentVar')[depth2].get('FieldRef.FieldPath'))
		...
		for depth1 in range(len(InitContainers)):
			...
			if InitContainers[depth1].get('InitContainerEnvironmentVar') is not None:
				for depth2 in range(len(InitContainers[depth1].get('InitContainerEnvironmentVar'))):
					if InitContainers[depth1].get('InitContainerEnvironmentVar')[depth2].get('FieldRef.FieldPath') is not None:
						self.add_query_param('InitContainer.' + str(depth1 + 1) + '.InitContainerEnvironmentVar.' + str(depth2 + 1) + '.FieldRef.FieldPath', InitContainers[depth1].get('InitContainerEnvironmentVar')[depth2].get('FieldRef.FieldPath'))
