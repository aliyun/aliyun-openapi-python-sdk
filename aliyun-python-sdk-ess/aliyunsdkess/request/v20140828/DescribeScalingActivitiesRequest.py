...
class DescribeScalingActivitiesRequest(RpcRequest):
	...
	def get_ScalingActivityIds(self): # RepeatList
		return self.get_query_params().get('ScalingActivityId')

	def set_ScalingActivityIds(self, ScalingActivityId):  # RepeatList
		for depth1 in range(len(ScalingActivityId)):
			self.add_query_param('ScalingActivityId.' + str(depth1 + 1), ScalingActivityId[depth1])
