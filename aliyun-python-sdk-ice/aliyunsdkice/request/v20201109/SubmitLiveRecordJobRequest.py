...
class SubmitLiveRecordJobRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'ICE', '2020-11-09', 'SubmitLiveRecordJob','ice')
	...
	def get_NotifyUrl(self): # String
		return self.get_body_params().get('NotifyUrl')

	def set_NotifyUrl(self, NotifyUrl):  # String
		self.add_body_params('NotifyUrl', NotifyUrl)
	def get_RecordOutput(self): # Struct
		return self.get_body_params().get('RecordOutput')

	def set_RecordOutput(self, RecordOutput):  # Struct
		self.add_body_params("RecordOutput", json.dumps(RecordOutput))
