...
class SaveBaseStrategyPeriodRequest(RpcRequest):
	...
	def get_OnlyWeekdays(self):
		return self.get_query_params().get('OnlyWeekdays')

	def set_OnlyWeekdays(self,OnlyWeekdays):
		self.add_query_param('OnlyWeekdays',OnlyWeekdays)

	def get_WorkingTimeFramesJson(self):
		return self.get_query_params().get('WorkingTimeFramesJson')

	def set_WorkingTimeFramesJson(self,WorkingTimeFramesJson):
		self.add_query_param('WorkingTimeFramesJson',WorkingTimeFramesJson)

	def get_WorkingTimes(self):
		return self.get_query_params().get('WorkingTime')

	def set_WorkingTimes(self, WorkingTimes):
		for depth1 in range(len(WorkingTimes)):
			if WorkingTimes[depth1] is not None:
				self.add_query_param('WorkingTime.' + str(depth1 + 1) , WorkingTimes[depth1])