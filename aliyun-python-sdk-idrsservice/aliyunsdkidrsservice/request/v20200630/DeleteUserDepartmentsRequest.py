...
class DeleteUserDepartmentsRequest(RpcRequest):

	def __init__(self):
		RpcRequest.__init__(self, 'idrsservice', '2020-06-30', 'DeleteUserDepartments','idrsservice')
	...
	def set_UserIds(self, UserIds):
		for depth1 in range(len(UserIds)):
			if UserIds[depth1] is not None:
				self.add_body_params('UserId.' + str(depth1 + 1) , UserIds[depth1])